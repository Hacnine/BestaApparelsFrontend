...
const FabricCostSectionEdit = ({ data, onChange }: FabricCostSectionEditProps) => {
  ...
  useEffect(() => {
    if (data?.yarnRows) setYarnRows(data.yarnRows);
    if (data?.knittingRows) setKnittingRows(data.knittingRows);
    if (data?.dyeingRows) setDyeingRows(data.dyeingRows);
  }, [data]);
