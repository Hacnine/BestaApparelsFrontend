          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-b hover:bg-muted/30 transition-colors">
                ...
                <td className="p-3 text-right">{row.unit ? Number(row.unit).toFixed(2) : "0.00"}</td>
                <td className="p-3 text-right">{row.rate ? Number(row.rate).toFixed(2) : "0.00"}</td>
                <td className="p-3 text-right">{row.value ? Number(row.value).toFixed(2) : "0.00"}</td>
              </tr>
            ))}
